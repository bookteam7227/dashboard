...
function renderDailyCharts(startDateId, endDateId) {
    ...
    status.textContent = `${formatFullDate(startDateId)} ~ ${formatFullDate(endDateId)} · 전년 동기간 비교`;
}

function populateMonthSelect() {
    const select = document.getElementById("monthSelect");
    const months = new Set(Array.from(dailyMap.keys()).map((dateId) => dateId.slice(0, 7)));
...
function applyDefaultPeriod() {
    ...
    const startDateId = dateToId(addDays(today, -60));
    const endDateId = dateToId(addDays(today, 60));
...
async function loadAnnualData() {
    ...
    annualChart = createAnnualChart(
        ...
        yearlyCourierData(rows)
    );
    const status = document.getElementById("annualStatus");
    status.textContent = `${yearlyCourierData(rows).size}개 연도 자료 · 범례를 누르면 해당 연도를 숨기거나 표시할 수 있습니다.`;
}
...
export async function mount({ content, actions }) {
    ...
    actions.innerHTML = `
        <div class="filters">
            ...
            <button id="searchButton" class="btn primary" type="button">조회</button>
            <div class="monthbox">
                <span class="monthbox-label">월별조회</span>
                <select id="monthSelect" aria-label="월별조회">
    ...
    content.innerHTML = `
        <section class="period-row">
            <p id="periodStatus" class="period-status">일별 출고자료를 불러오고 있습니다.</p>
        </section>
        ...
        <section class="chart-grid">
            <article class="chart-card">
                <div class="chart-head">
                    <div>
                        <div class="chart-title">전체 출고 건수</div>
                        <div class="chart-sub">금년 vs 전년 동기간</div>
                    </div>
                    <div class="chart-legend">
                        <span class="legend-item"><span class="legend-line legend-current"></span>금년</span>
                        <span class="legend-item"><span class="legend-line legend-previous"></span>전년 동기간</span>
                    </div>
                </div>
            ...
            <article class="chart-card">
                <div class="chart-head">
                    <div>
                        <div class="chart-title">전체 출고 권수</div>
                        <div class="chart-sub">금년 vs 전년 동기간</div>
                    </div>
                    <div class="chart-legend">
                        <span class="legend-item"><span class="legend-line legend-current"></span>금년</span>
            ...
            <article class="chart-card">
                <div class="chart-head">
                    <div>
                        <div class="chart-title">연간 택배 발송 건수</div>
                        <div class="chart-sub">1월~12월 월간 합계 · 연도별 비교</div>
                    </div>
                    <p id="annualStatus" class="annual-status">월간 택배자료를 불러오고 있습니다.</p>
                </div>
                <div class="chart-box annual-chart-box"><canvas id="annualCourierChart"></canvas></div>
            </article>
        </section>
    `;

    document.getElementById("searchButton").addEventListener("click", searchByManualPeriod);
    ...
    if (dailyResult.status === "rejected") {
        ...
        const status = document.getElementById("periodStatus");
        status.classList.add("error");
        status.textContent = "일별 출고자료를 불러오지 못했습니다. Firestore 설정과 보안 규칙을 확인해 주십시오.";
    }
    if (annualResult.status === "rejected") {
        console.error("[monthly_courier_statistics]", annualResult.reason);
        const status = document.getElementById("annualStatus");
        status.classList.add("error");
        status.textContent = "연간 택배자료를 불러오지 못했습니다. Firestore 읽기 권한을 확인해 주십시오.";
    }
}
